...
def train(model, train_dataloader, eval_dataloader, epochs, learning_rate, device, surrogate=False, model_name='model.bin'):
...
def evaluate(model, device, eval_dataloader):
    ...
    labels_all = []
    time_count = []
    print("N. Batches: ", len(eval_dataloader))
    with torch.no_grad():
...
def distill(hyperparams_set, eval=False, surrogate=True, model_name='model.bin', seed=1):
    ...
    for hyperparams in hyperparams_set:
        ...
        if not eval:
            ...
            model.to(device)
            dev_best_acc, pred_original = train(model, train_dataloader, eval_dataloader, epochs, learning_rate, device,
                                                surrogate)
            ...
            model_dir = os.path.join("../checkpoints", "Avatar", model_name)
