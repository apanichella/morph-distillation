        ...
        if dev_acc >= dev_best_acc:
            ...
            if not surrogate:
                ...
                if os.path.exists(model_path):
                    ...
                    logger.info(f"Existing model file {model_path} deleted.")
        ...
        if not eval:
            ...
            logger.info(
                "Test Acc: {0}, Test Precision: {1}, Test Recall: {2}, Test F1: {3}".format(test_results["eval_acc"],
                                                                                            ...
                                                                                            test_results[
                                                                                                "inference_time"]))
            #n_flips = np.sum(prediction != pred_metamorphic)
            #prediction_flips.append(n_flips)
            dev_best_accs.append(test_results["eval_acc"])
            logger.info("N. Prediction flips: {0}".format(0))
